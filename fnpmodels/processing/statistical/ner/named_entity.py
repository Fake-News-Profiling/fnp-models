...
from ..extractor import TweetStatsExtractor, clean_text
...
def named_entities_counts(tweet_feed: List[str], ner_tagger: AbstractNerTaggerWrapper = None) -> List[int]:
    ...
    for tweet in tweet_feed:
        cleaned_tweet = clean_text(tweet, remove_punc=False, remove_digits=False, remove_tags=True)
...
def ner_tweet_extractor(ner_wrapper: AbstractNerTaggerWrapper) -> TweetStatsExtractor:
    """ Create a TweetStatsExtractor for named entity recognition features """
    extractor = TweetStatsExtractor([partial(named_entities_counts, ner_tagger=ner_wrapper)])
