...
from fnpmodels.processing.statistical import readability, ner, sentiment
from fnpmodels.experiments.bert.bert_combined import (
    BertDownstreamLossLogitsCombinedExperiment,
    BertDownstreamLossPooledCombinedExperiment,
)
from fnpmodels.experiments.experiment import AbstractSklearnExperiment, ExperimentConfig
from fnpmodels.experiments.handler import ExperimentHandler
from fnpmodels.experiments.statistical import get_sentiment_wrapper, get_ner_wrapper
...
class AbstractEnsembleExperiment(AbstractSklearnExperiment):
    ...
    def input_data_transformer(self, x):
        ...
        tweet_preprocessor = TweetPreprocessor([tag_indicators, replace_xml_and_html])
        x_preprocessed = tweet_preprocessor.transform(x)

        # Readability stats
        read_extractor = readability.readability_tweet_extractor()
        x_read = read_extractor.transform(x_preprocessed)

        # Sentiment stats
        sent_extractor = sentiment.sentiment_tweet_extractor(get_sentiment_wrapper(self.child_hyperparameters["sentiment"]))
        x_sent = sent_extractor.transform(x_preprocessed)

        # NER stats
        ner_extractor = ner.ner_tweet_extractor(get_ner_wrapper(self.child_hyperparameters["ner"]))
