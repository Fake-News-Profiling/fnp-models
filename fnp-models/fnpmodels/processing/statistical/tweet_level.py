...
from .sentiment import AbstractSentimentAnalysisWrapper

""" Tweet-level data extraction functions """


def tweet_level_extractor(sentiment_wrapper: AbstractSentimentAnalysisWrapper) -> ex.TweetStatsExtractor:
...
def emojis_count(tweet: str) -> int:
    ...
    return len(ex.emoji_chars([tweet])[0])


def syllables_count(tweet: str) -> int:
    """ Returns the number of syllables in this tweet """
    tweet_words = ex.tweets_to_words([tweet], remove_tags=True)[0]
    tweet_syllables = sum(map(ex.syllables, tweet_words))
    return tweet_syllables


def tweet_lengths(tweet: str) -> List[int]:
    """ Return the length of the tweet, in words and characters """
    num_words = len(ex.tweets_to_words([tweet], remove_tags=True)[0])
    num_chars = len(ex.clean_text(tweet, remove_tags=True))
...
def capitalisation_counts(tweet: str) -> List[int]:
    ...
    clean_tweet = ex.clean_text(tweet, remove_digits=False, remove_tags=True)
